...
install_python "3.13.7"
#install_python "3.12.11"
echo "Installation complete!"#!/bin/sh
...
#         #Ensures openssl can be found:
#         export CPPFLAGS="-I/usr/pkg/include" export LDFLAGS="-L/usr/pkg/lib -Wl,-R/usr/pkg/lib" export PKG_CONFIG_PATH="/usr/pkg/lib/pkgconfig" export CFLAGS="-std=gnu99 -D_GNU_SOURCE"
        
#         echo "_ctypes _ctypes/*.c" >> Modules/Setup.local echo "CFLAGS+= -std=gnu99 -D_GNU_SOURCE" >> Modules/Setup.local
        
#         echo "Configuring the build with optimizations..." ./configure --prefix=/usr/local --enable-optimizations --with-ensurepip=upgrade --with-openssl=/usr/pkg --with-openssl-rpath=/usr/pkg/lib echo 
#         "Building Python $VERSION" gmake -j profile-opt gmake altinstall if [ ! -x "$PYTHON_BIN" ]; then
#                 echo "Build or installation failed for Python $VERSION" fi
        
#         echo "Cleaning up build files..." cd .. rm -rf "$PYTHON_SRC" "$PYTHON_TGZ" echo "Verifying installation..." "python${MAJOR_MINOR}" --version
        
#         echo "Installing pyperformance for $PYTHON_BIN..."
#         "python${MAJOR_MINOR}" -m ip install --upgrade pip
#         "python${MAJOR_MINOR}"  -m pip install pyperformance
